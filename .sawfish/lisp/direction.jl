...
;;;; (C) Ives Aerts <ives.aerts@sonycom.com>, 1 Aug 2000
...
(defun direction-north (w)
  ...
  (let* ((wy (cdr (window-position w)))
         (dfocusable-windows (filter (lambda (w)
                                        (and (window-dfocusable-p w)
                                             (< (cdr (window-position w)) wy)))
                                     (managed-windows))))
...
(defun focus-west (w)
  ...
  (let ((nearest (direction-west w)))
       (when nearest (set-input-focus nearest)) ) )
